
#!/bin/bash

set -e

QEMU_VERSION="6.2.0"
QEMU_URL="https://download.qemu.org/qemu-${QEMU_VERSION}.tar.xz"
TARBALL="qemu-${QEMU_VERSION}.tar.xz"
EXTRACT_DIR="qemu-${QEMU_VERSION}"
FILES_DIR="files"
ASSETS_DIR="assets"

echo "Creating directories..."
mkdir -p "${FILES_DIR}" "${ASSETS_DIR}"

# Download QEMU if not already present
if [ -f "${FILES_DIR}/${TARBALL}" ]; then
    echo "QEMU tarball already downloaded, skipping download..."
else
    echo "Downloading QEMU ${QEMU_VERSION}..."
    curl -L -o "${FILES_DIR}/${TARBALL}" "${QEMU_URL}"
fi

# Extract if not already extracted
if [ -d "${FILES_DIR}/${EXTRACT_DIR}" ]; then
    echo "QEMU already extracted, skipping extraction..."
else
    echo "Extracting tarball..."
    tar -xf "${FILES_DIR}/${TARBALL}" -C "${FILES_DIR}"
fi

echo "Copying edk2-aarch64-code.fd.bz2 from pc-bios..."
cp "${FILES_DIR}/${EXTRACT_DIR}/pc-bios/edk2-aarch64-code.fd.bz2" .
cp "${FILES_DIR}/${EXTRACT_DIR}/pc-bios/edk2-arm-vars.fd.bz2" .

echo "Decompressing edk2-aarch64-code.fd.bz2..."
bunzip2 edk2-aarch64-code.fd.bz2
bunzip2 edk2-arm-vars.fd.bz2

echo "Moving edk2-aarch64-code.fd to assets..."
mv edk2-aarch64-code.fd "${ASSETS_DIR}/"
mv edk2-arm-vars.fd "${ASSETS_DIR}/"

echo "Done! edk2-aarch64-code.fd and edk2-arm-vars.fd are in ${ASSETS_DIR}/"